...
ConvertSelection(ThisHotkey) {
    ...
    ; --- NEW: Switch Keyboard Layout ---
    if (ContainsHebrew(convertedText)) {
        SetKeyboardLayout(0x040D) ; Hebrew
    } else {
...
SetKeyboardLayout(langID) {
    ; PRO TIP: 0x50 is WM_INPUTLANGCHANGEREQUEST.
    ; We send this message to the ACTIVE window (A) to request a layout change.
    try PostMessage(0x50, 0, langID,, "A")
}
